...
async def create_url(db: AsyncSession, url_create: URLCreate, user_id: Optional[int] = None) -> URL:
...
async def get_urls_by_user(db: AsyncSession, user_id: int, skip: int = 0, limit: int = 100):
...
async def get_url_by_id_and_user(db: AsyncSession, url_id: int, user_id: int) -> Optional[URL]:
